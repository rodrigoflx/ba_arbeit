...
static void output_csv(struct node *nodes, unsigned long nnodes, unsigned long nranges)
{
	unsigned long i, j, cur_vals, interval_step, next_interval;
	...
	printf("entry, cnt, rel_freq\n");
	for (i = 0; i < output_nranges; i++) {
		struct output_sum *os = &output_sums[i];
		printf("%lu, %u, %.6f\n", i, os->nranges, os->output);
	}
...
int main(int argc, char *argv[])
{
	...
	if (hash == NULL) {
		printf("Failed to allocate memory for %lu hash\n", hash_size);
		return -1;
	} else {
		printf("Allocated %lu hash\n", hash_size);
	}
	...
	if (nodes == NULL) {
		printf("Failed to allocate memory for %llu nodes\n", nranges);
		return -1;
	} else {
		printf("Allocated %llu nodes\n", nranges);
	}
	...
	if (output_type == OUTPUT_CSV)
		output_csv(nodes, nnodes, nranges);
	else
